//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

//
// SDK Root: /Developer/SDKs/MacOSX-I.sdk
//

#import <IDEKit/IDEEditorDocument.h>

#import <IDEKit/IDETestReportPackagedGeniusResultDocumentRequirements-Protocol.h>

@class IDEResultBundleProcessor, IDESchemeActionsInvocationRecord, NSArray, NSString;

@interface IDEResultBundleEditorDocument : IDEEditorDocument <IDETestReportPackagedGeniusResultDocumentRequirements>
{
    IDESchemeActionsInvocationRecord *_invocationRecord;
    IDEResultBundleProcessor *_processor;
}

+ (BOOL)shouldUnlockFileURLBeforeMakingChanges;
@property(retain, nonatomic) IDEResultBundleProcessor *processor; // @synthesize processor=_processor;
@property(retain, nonatomic) IDESchemeActionsInvocationRecord *invocationRecord; // @synthesize invocationRecord=_invocationRecord;
- (void).cxx_destruct;
@property(readonly, nonatomic) NSString *onDiskPath;
@property(readonly, nonatomic) NSString *uniqueIdentifier;
@property(readonly, nonatomic) NSArray *testableSummaries;
- (void)editorDocumentDidClose;
- (BOOL)readFromURL:(id)arg1 ofType:(id)arg2 error:(id *)arg3;
- (void)extractInvocationRecordFromFilePath:(id)arg1;
- (int)readOnlyStatus;
- (BOOL)canSave;

@end

