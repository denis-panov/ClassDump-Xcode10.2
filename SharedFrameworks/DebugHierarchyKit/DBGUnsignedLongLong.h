//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

//
// SDK Root: /Developer/SDKs/MacOSX-I.sdk
//

#import <objc/NSObject.h>

#import <DebugHierarchyKit/DBGValue-Protocol.h>

@class NSString;

@interface DBGUnsignedLongLong : NSObject <DBGValue>
{
    unsigned long long _unsignedLongLongValue;
}

+ (id)withValue:(unsigned long long)arg1;
@property unsigned long long unsignedLongLongValue; // @synthesize unsignedLongLongValue=_unsignedLongLongValue;
- (id)objectValue;
- (id)initWithUnsignedLong:(unsigned long long)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

